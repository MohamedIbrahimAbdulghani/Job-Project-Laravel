...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    <head>
        ...
        <title>Job Board - {{ $title ?? "Laravel"}} </title>
    ...
    <body>
        {{-- This is layout from tailwind framework --}}
        <div class="min-h-full">
            ...
            <div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
                ...
                <div class="hidden md:block">
                    <div class="ml-4 flex items-center md:ml-6">
                            {{-- Authentication --}}
                            @auth
                                <span style="color: white; margin-left: 10px; margin-right: 10px">{{Auth::user()->name}}</span>
                                <form action="{{url('logout')}}" method="post">
                                    @csrf
                                    <button class="btn btn-danger ml-3 ">Logout</button>
                                </form>
                            @else
                                <a href="{{url('signin')}}" class="btn btn-primary mr-3">Sign In</a>
            ...
            <div class="mx-auto max-w-7xl px-4 py-6 sm:px-6 lg:px-8">
                {{ $slot }}  {{--This is variable to know component i will write some code inside this area --}}
            </div>
        </div>
